        ...
        if ($VmSelected)
        {
            ...
            while ((-not $Kek) -and ($Index -lt $VmSelected.Count))
            {
                ...
                if ($Vm.StorageProfile.OsDisk.EncryptionSettings -eq $null)
                {
                    ...
                    if($Disks[0].EncryptionSettings -ne $null)
                    {
                        $Bek = $Disks[0].EncryptionSettings[0].DiskEncryptionKey
                        $Kek = $Disks[0].EncryptionSettings[0].KeyEncryptionKey
                    }
                    else
                    {
                        throw [System.MissingFieldException] "Virtual machine $VmName encrypted but encryption " + `
